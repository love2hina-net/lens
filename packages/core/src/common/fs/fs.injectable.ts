...
const fsInjectable = getInjectable({
  ...
  instantiate: () => {
    ...
    return {
      readFile,
      readJson: readJson as (file: string, options?: JFReadOptions | BufferEncoding) => Promise<any>,
      writeFile,
      writeJson,
      pathExists,
      readdir,
      ...
      ensureDir: ensureDir as (path: string, options?: number | fse.EnsureDirOptions) => Promise<void>,
      ensureDirSync,
      createReadStream,
      stat,
      unlink,
